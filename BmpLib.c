...
/*Fonction qui prend une image en parametre, alloue un tableau 3D et remplis ce tableau de l'image (pixel par pixel) dans l'ordre B V R*/
unsigned char*** cree3Matrices(DonneesImageRGB *monImage) 
{
    int i,j,k = 0;
    ...
    for(i=0 ; i < 3 ; i++)
    {
        tableau3D[i] = malloc(monImage->largeurImage * sizeof(**tableau3D));
    }
    for(i=0 ; i < 3 ; i++)
    {
        for(j=0 ; j < monImage->largeurImage ; j++)
        {
           tableau3D[i][j] = malloc(monImage->hauteurImage * sizeof(***tableau3D));
        }
    }

    /* On découpe l'image de notre structure dans notre tableau 3D */
    for ( i = 0; i < monImage->hauteurImage; i ++ )
    {
        for ( j = 0 ; j < monImage->largeurImage; j ++)
        {
                tableau3D[0][j][i]= monImage->donneesRGB[k]; //b
                tableau3D[1][j][i]= monImage->donneesRGB[k+1]; //v
                tableau3D[2][j][i]= monImage->donneesRGB[k+2]; //r
                k=k+3;
        }
    }
    return tableau3D;
...
/*Fonction qui prend en parametre un tableau 3D et forme une image a l'aide de ce tableau (retourne une image de type DonneesRGB*/
DonneesImageRGB* creeImage(unsigned char*** tableau3D, int largeur, int hauteur) 
{
    int i,j,k;

    DonneesImageRGB* tableauBase = malloc(sizeof(DonneesImageRGB));
   tableauBase ->donneesRGB = (unsigned char *) malloc(sizeof(unsigned char) * (largeur* hauteur) * 3);

    for ( i = 0, k=0 ; i < hauteur; i ++ )
    {
        for ( j = 0 ; j < largeur; j ++)
        {
            tableauBase ->donneesRGB[k]=tableau3D [0][j][i];
           tableauBase ->donneesRGB[k+1]=tableau3D [1][j][i];
            tableauBase->donneesRGB[k+2]=tableau3D [2][j][i];
            k=k+3;
        }
    }
   tableauBase ->largeurImage=largeur;
    tableauBase ->hauteurImage=hauteur;
